...
class MPSQLiteCursorAttributesProxy:
    ...
    def __call__(self, *args, **kwargs):
        ...
        self.__cursor_request_queue.put(MPSQLiteCursorRequest(request_id, self.__cursor_id, self.__cursor_args, self.__cursor_kwargs, "attr", self.__name, args, kwargs))
        while True:
            try:
    ...
    def __iter__(self):
        request_id = str(uuid.uuid4())
        self.__cursor_request_queue.put(MPSQLiteCursorRequest(request_id, self.__cursor_id, self.__cursor_args, self.__cursor_kwargs, "iter", self.__name, (), {}))
        while True:
            try:
                response: MPSQLiteCursorResponse = self.__cursor_response_queue.get()
...
class MPSQLiteCursorWrapper:
    ...
    def __getattr__(self, name):
        return MPSQLiteCursorAttributesProxy(id(self), self.__args, self.__kwargs, name, self.__cursor_request_queue, self.__cursor_response_queue)